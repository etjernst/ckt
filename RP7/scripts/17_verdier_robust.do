...
file write mdh "" _n
file write mdh "Each per-country block reports $\hat\phi^{\mathrm{rob}}$, $\Delta_{\text{never}}^{\mathrm{rob}}$, $\Delta_{\text{always}}^{\mathrm{rob}}$, average $\Delta^{\mathrm{rob}}$, and convergence indicator across the 5 progressive covariate specifications x 2 GMM step variants." _n
