...
* Baseline-vs-cluster-residualized GRC slope summary table.
* Reads phi (`coefname', default phi:_cons) from two .ster per country:
*   baseline  -> $output/`baseprefix'`country'`basesuffix' (default grc_<c>_cuu_ca)
*   cluster   -> $output/`vvprefix'`country'`vvsuffix'     (default vv_<c>_os_covs_all)
* and writes a country-by-row LaTeX table. Significance stars are computed
* from the normal approximation z = b/se, matching the star levels used by
* the esttab-based table writers (* .10, ** .05, *** .01).
* **********************************************************************
capture program drop cluster_comparison_table
program define cluster_comparison_table
    syntax , FILEname(string) [                              ///
        Countries(string) coefname(string)                  ///
        baseprefix(string) basesuffix(string)               ///
        vvprefix(string)   vvsuffix(string) ]

    if "`countries'"  == "" local countries "IDN CHN TZA"
    if "`coefname'"   == "" local coefname  "phi:_cons"
    ...
    file write `fh' "\caption{Baseline versus cluster-residualized GRC slope, consumption (unbalanced panel)}" _n
    file write `fh' "\label{tab:verdier-robust}" _n
    file write `fh' "\begin{tabular}{l cc} \toprule" _n
    file write `fh' "                       & \multicolumn{1}{c}{Baseline \$\hat\phi\$}" _n
    file write `fh' "                       & \multicolumn{1}{c}{Cluster-residualized \$\hat\phi^{\mathrm{rob}}\$} \\" _n
    ...
    foreach c of local countries {
        ...
        * Baseline phi
        capture quietly estimates use "$output/`baseprefix'`c'`basesuffix'"
        if _rc {
            di as error "cluster_comparison_table: missing baseline ster ($output/`baseprefix'`c'`basesuffix')"
            file close `fh'
            exit 198
        }
        local bb  = _b[`coefname']
        local bse = _se[`coefname']
        local bstar = ""
        local bp = 2*normal(-abs(`bb'/`bse'))
        if `bp' < 0.10 local bstar "*"
        if `bp' < 0.05 local bstar "**"
        if `bp' < 0.01 local bstar "***"

        * Cluster-residualized phi
        capture quietly estimates use "$output/`vvprefix'`c'`vvsuffix'"
        if _rc {
            di as error "cluster_comparison_table: missing cluster ster ($output/`vvprefix'`c'`vvsuffix')"
            file close `fh'
            exit 198
        }
        local rb  = _b[`coefname']
        local rse = _se[`coefname']
        local rstar = ""
        local rp = 2*normal(-abs(`rb'/`rse'))
        if `rp' < 0.10 local rstar "*"
        if `rp' < 0.05 local rstar "**"
        if `rp' < 0.01 local rstar "***"

        local bbs  = trim(string(`bb',  "%9.3f"))
        local rbs  = trim(string(`rb',  "%9.3f"))
        local bses = trim(string(`bse', "%9.3f"))
        local rses = trim(string(`rse', "%9.3f"))

        file write `fh' "`cname' & `bbs'`bstar' & `rbs'`rstar' \\" _n
        file write `fh' " & (`bses') & (`rses') \\" _n
