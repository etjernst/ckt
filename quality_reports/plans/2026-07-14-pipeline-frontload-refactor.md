...
Deviation from the plan text, chosen for equivalence: the 999 sentinel is NOT written into the saved data, because `3_OLS_uGRC`/`6_OLS_uGRC_hukou` run `i.trajectory` regressions without calling `setup_grc_estimation`, and a stored sentinel would pull every unbalanced individual into those fits as a new factor level; `trajectory` stays missing in the file and the sentinel recode remains a documented load-time step in the reader (the inversion's Python bridge also sees the in-memory 999, so estimation-time state is bitwise today's).
