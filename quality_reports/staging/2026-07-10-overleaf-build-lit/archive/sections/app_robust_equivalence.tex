%==============================================================================
%  Appendix: proof of Proposition 2 (equivalence under location pooling).
...
\section{Equivalence with Verdier's worker-level estimator}\label{app:verdier-equivalence}
...
To accommodate location-level heterogeneity not absorbed by trajectory or treatment, we work with the generalization of Equation~\eqref{eq:generalized-consumption-eq-simple} that adds a location-level intercept $c(v_i)$:
...
Setting $c(v_i) \equiv 0$ recovers the baseline model used in Section~\ref{subsec:empirical-model}; the location-residualized estimator's reason for existence is that consistency does not require $c(v_i) \equiv 0$.
...
\]
Substituting the LCA $\Delta_i = \beta + \phi\theta_i$ into the projection of $\theta_i$ onto $a_i - c(v_i) = \alpha_i$ within location $v_i$ delivers
...
The location-conditional zero-mean property in \eqref{app:eq:worker-lca} follows from the within-location projection that defines $\xi_i$ together with (A1) (time-invariance of $\theta_i,\tau_i$).
...
\end{equation}
where $\tilde z_i$ is a vector of location-demeaned period-treatment indicators: for each $t$, $\tilde z_{it} = D_{it} - E[D_{it}\mid v_i]$, with $\tilde z_i$ collecting the $T$ entries (or any subset that achieves identification under (A4)).

Our trajectory-pooled estimator $\hat\phi^{\mathrm{rob}}$ solves the GMM moment of Equation~\eqref{eq:restricted-grc-unbalanced} with the location-residualized instruments of Section~\ref{subsec:verdier-robustness}, i.e.,
...
\subsection*{Step 2.  Consistency of \texorpdfstring{$\hat\alpha_1$}{alpha\_1-hat} for \texorpdfstring{$1/\phi$}{1/phi}.}
...
\subsection*{Step 3.  Consistency of \texorpdfstring{$\hat\phi^{\mathrm{rob}}$}{phi-hat-rob} for \texorpdfstring{$\phi$}{phi}.}
...
The third term, $\xi_i^{\,*}\,D_{it}$, has $E[\tilde D_{it}^{\,\underline d}\,\xi_i^{\,*}\,D_{it}] = 0$ by (A2)--(A4): treatment is exogenous within location.
...
The rank condition for identification---that $\tilde D_{it}^{\,\underline d}$ has non-trivial within-location variation across switcher trajectories---is implied by (A4).
...
Standard cluster asymptotics deliver $\sqrt V$-rate convergence at the location level for both estimators; the variance comparison depends on the within-location cross-distribution of trajectories and is not addressed here.

Where Assumption~\ref{ass:cluster-pooling} fails, the first-term argument in Step 3 breaks: $E[\tilde D_{it}^{\,\underline d}(\alpha_i + c(v_i) - \mu_{\underline d})]$ no longer vanishes when the within-location distribution of $\alpha_i$ varies systematically across trajectories.
Tracing through the algebra recovers the location-pooling bias formula reported in Equation~\eqref{eq:cluster-pooling-bias}.
The worker-level argument in Step 2 is unaffected because it operates within location.
