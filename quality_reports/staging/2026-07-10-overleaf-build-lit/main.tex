...
Our baseline results pool balanced and unbalanced observers: the latter are absorbed into a single pooled cell through the $U_i$ indicator and its interaction with the urban dummy.
Proposition~\ref{prop:pooling} in Appendix~\ref{app:pooling} shows that this pooling is consistent under our identifying assumptions and a standard missing-at-random condition on attrition. 
% It also shows that the balanced-panel trajectory estimates remain consistent even when attrition in the unbalanced stratum is non-ignorable.
Below, we nonetheless verify empirically that the baseline estimates do not depend on partial observers by re-estimating the restricted GRC on the balanced subsample.\footnote{The close correspondence between pooled and balanced-only trajectory estimates provides an empirical check on the joint null that the orthogonality and common-covariate-effects conditions of Appendix~\ref{app:pooling} both hold.}
...
\input{unbalanced_proposition_short}
\input{robust_equivalence_proof}
