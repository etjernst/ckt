...
We include an ``unbalanced'' dummy variable that takes a value of one for individuals with at least one missing round of data, as well as the interaction between the ``unbalanced'' dummy and the urban indicator.\footnote{Since unbalanced individuals lack complete trajectories, all trajectory indicators equal zero for them. They therefore do not contribute to identifying the trajectory-specific parameters $\mu_{\underline{d}}$ or the slope $\phi$, and enter estimation only through the covariate coefficients $\gamma$ and the pooled unbalanced-cell parameters $(\mu_{\mathrm{unb}},\Delta_{\mathrm{unb}})$, where $\mu_{\mathrm{unb}}$ is the rural-consumption mean and $\Delta_{\mathrm{unb}}$ the average urban return on the unbalanced stratum. Proposition~\ref{prop:pooling} formalizes this approach and shows that, under a missing-at-random condition and a common-covariate-effects condition (Assumptions~\ref{ass:mar} and \ref{ass:common-gamma}), the resulting pooled estimator is $\sqrt n$-consistent, asymptotically normal, and weakly more efficient for the trajectory parameters than the analogous estimator restricted to balanced individuals.}
...
Our baseline results pool balanced and unbalanced individuals: the latter are absorbed into a single pooled cell through the $U_i$ indicator and its interaction with the urban dummy.
Proposition~\ref{prop:pooling} in Appendix~\ref{app:pooling} shows that under our identifying assumptions and a standard missing-at-random condition on attrition, this pooling is consistent and asymptotically normal, and is weakly more efficient than the balanced-only estimator for the trajectory parameters and covariate effects.
The unbalanced stratum enters the estimating equation only through the covariate vector $\gamma$ and the pooled unbalanced-cell parameters $(\mu_{\mathrm{unb}},\Delta_{\mathrm{unb}})$; it does not contribute directly to the identification of any trajectory-specific return.
The close correspondence between pooled and balanced-only trajectory estimates therefore constitutes a direct empirical check on the joint null that missingness is conditionally at random and that covariate effects are common across strata.
We re-estimate the restricted GRC on the balanced subsample to verify this.
...
\newpage
\section{Consistency of the pooled estimator}\label{app:pooling}

\input{unbalanced_proposition}
