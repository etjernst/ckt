...
Tanzania delivers the largest, $[+14.3\%, +22.7\%]$.
China lands between them at $[+7.8\%, +8.8\%]$, computed as the population-weighted aggregate of the rural-hukou-first ($74\%$ of CHN individuals with defined hukou status) and urban-hukou-first ($26\%$) subsamples; we do not report a pooled CHN gap because the pooled $J$-test rejects on the unbalanced sample.
...
The rural-hukou-first subsample carries a misallocation gap of $[+10.5\%, +11.7\%]$, comparable in magnitude to Tanzania's.
The urban-hukou-first subsample carries $[+0.9\%, +1.3\%]$, an order of magnitude smaller.
...
Without the fallback, the upper bounds inflate to $+45.6\%$ for Indonesia, $+171\%$ for Tanzania, and several orders of magnitude for the China urban-hukou-first subsample, driven by the singularity rather than honest uncertainty.
