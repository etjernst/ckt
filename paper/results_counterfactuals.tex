...
The exercise sits in a tradition of structural migration counterfactuals---\cite{bryanAggregateProductivityEffects2019} on the aggregate productivity gain from removing rural-urban frictions in Indonesia, \cite{lagakosMigrationCostsObservational2020} on the migration costs implied by observed wage gaps in panel data, and \cite{lagakosUrbanRuralGapsDeveloping2020a} on the urban-rural productivity gap more broadly---and at the intersection with the agricultural-misallocation strand exemplified by \cite{adamopoulosMisallocationSelectionProductivity2022} and the broader Hsieh--Klenow tradition.%
\footnote{We use ``misallocation gap'' in the standard sense of the gap between observed and counterfactually-optimal allocation; our object differs from a Hsieh-Klenow TFPR-dispersion index in that it is identified from panel switchers under the LCA restriction rather than from cross-sectional marginal-revenue-product dispersion.}
What our identification delivers that distinguishes the exercise: trajectory-conditional returns identified from switchers and extrapolated to non-switchers through the LCA restriction, with inference propagated through the inversion confidence region rather than asymptotic standard errors that the identification boundary at $\phi = -1$ would make unreliable.
...
At one end, a no-further-migration scenario in which workers stay in the location of their initial trajectory observation regardless of $\Delta_i$.
...
This decomposition by panel trajectory is what the existing aggregate-gain literature does not deliver: under our pro-poor finding the never-migrant piece dominates the gap in TZA, the lumped-switcher cell dominates the value of observed migration in IDN, and isolating where the misallocation lives is the cleanest way to communicate the substantive content of the magnitude.

The estimated misallocation gap differs sharply across the three countries.
...
China lands between them at $[+7.5\%, +8.8\%]$, computed as the population-weighted aggregate of the rural-hukou-first ($74\%$ of CHN individuals with defined hukou status) and urban-hukou-first ($26\%$) subsamples; we do not report a pooled CHN gap because the pooled $J$-test rejects on the unbalanced sample.
All three intervals are convex hulls of the joint $(\phi, \beta)$ confidence region propagated through equation~\eqref{eq:misallocation-decomposition}, not asymptotic delta-method intervals.

The Chinese national figure masks an institutional asymmetry between the two hukou regimes.
...
\cite{gaiRuralPensionsLabor2025} extend this template using the staggered NRPS rollout as an identifying shock to migration costs, embedded in a generalized-Roy general-equilibrium framework that builds on the Fan (2019) Hukou Index; their hypothetical full hukou liberalization raises GDP by 2.04 log points.
Relative to these exercises, our contribution is a partial-equilibrium consumption-side magnitude disciplined by GRC-identified objects, with a trajectory-level decomposition that the existing literature does not deliver.
...
The intercept gap $\beta^{rh} - \beta^{uh}$ absorbs four sources in principle: the pecuniary urban premium net of cost-of-living, cost-of-living differences between the regions where the two regimes' workers tend to reside, compensating differentials for non-pecuniary location amenities, and any residual selection not captured by $\phi\theta_i$.
...
Identifying the scale $\sigma_\eta$ from the trajectory-share moment $\pi_{d_T} = E_\theta[(1 - F_\eta(-\Delta(\theta)))^T]$ is feasible but is left to ongoing work; for now we report the resorting magnitude as a function of $\sigma_\eta$ on a justified grid, in parallel under a type-I extreme value distribution (logit-style choice probabilities, the empirical migration default) and a normal distribution (probit-style, the Heckman selection convention).
...
We leave this welfare-side extension to future work.
