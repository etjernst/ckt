* ============================================================
* Title:   Step 0.5 stability test: 5x summclust at J=2000 then 5x at J=5000.
* Author:  Emilia (with Claude)
* Date:    2026-05-05
* Purpose: Probe summclust wall-time behavior at two J levels in one
*          session, with the system monitor PowerShell capturing CPU/mem
*          throughout. Same fixed seed each replicate so subsamples and
*          active-z sets are reproducible. Five reps at each J.
* Input:   IDN_unb.dta, grc_IDN_urban_covs_trend.ster (for phi_hat)
* Output:  output/stability_dual_<stamp>.csv  (one row per replicate, J in column)
*          output/stability_dual_<stamp>.log
* Notes:   Run via:  stata-mp -e do stability_test_dual.do
*          Same setup as stability_test_J5000.do, just with an outer J loop.
* ============================================================
...
log using "stability_dual_`stamp'.log", replace text

local csvfile "stability_dual_`stamp'.csv"
...
capture noisily {
    ...
    di as txt _n "============================================================"
    di as txt "Dual stability test: 5x J=2000 then 5x J=5000, IDN `balance'"
    di as txt "============================================================"
    ...
    foreach J of numlist 2000 5000 {
        di as txt _n _n "############################################################"
    ...
    di as txt _n "============================================================"
    di as txt "Dual stability test complete. CSV: `csvfile'"
    di as txt "============================================================"
}
...
capture log close
if `saved_rc' != 0 {
    di as error ">>> stability_test_dual.do FAILED with rc=`saved_rc'"
}
