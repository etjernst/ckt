"""V3 RF GMM-point near-rejection diagnostic.

The CHN-RF V3 run reported the GMM point sitting just outside the joint
CI: constrained Wald 18.87 vs threshold chi^2_{10, 0.95} = 18.31,
p = 0.042. The GMM Hansen J on the same parent ster was 12.64 on 8 dof,
p = 0.124 -- comfortably non-rejecting. The two tests are different
statistics on different parameterizations, but the discrepancy is
suspicious and the RF numbers feed the national CHN E1 headline.

This driver mirrors 2026-05-20_e1_v3_wald_at_gmm.py (IDN), but for the
CHN_hukou_rural_first subsample. The matched-spec auxiliary OLS, the
constrained-J Wald at the GMM point, the per-switcher moment
decomposition, and a marginal-CI cross-check via grid_lca_inversion.

Three readings of the Wald-at-GMM number:
  - Wald << threshold: spec is right and the prior 18.87 was a finite-
    sample artifact near the boundary; no further action needed.
  - Wald near threshold: real but tight tension between aux-OLS and GMM;
    investigate per-switcher contributions.
  - Wald >> threshold: bug or deeper spec mismatch.
"""
...
COUNTRY_FILE_STEM = "CHN_hukou_rural_first"
COUNTRY_SHORT = "CHN_rf"
BASE_TRAJECTORY = 2
THRESHOLD = 5
TYPE_ONE = 0.05


def load_scalars(country_short: str) -> dict:
    df = pd.read_csv(INPUTS_DIR / f"{country_short}_e1_scalars.csv")
    out: dict = {}
...
def main() -> None:
    print(f"{'=' * 64}")
    print(f"V3 Wald-at-GMM diagnostic: {COUNTRY_SHORT} col 5 (cuu_ca)")
    print(f"{'=' * 64}")

    scalars = load_scalars(COUNTRY_SHORT)
    ...
    df = df.dropna(subset=["lndepvar", "education_max", "age"])
    ...
    contrib = np.array([m[k] * (V_m_inv @ m)[k] for k in range(K)])
    print(f"\n  Per-switcher Wald contribution (m_s * (V_m^-1 m)_s):")
    print(f"    {'s':>4} {'contrib':>10} {'cum%':>8}")
    cum = 0.0
    ...
    for idx in order:
        ...
        c = contrib[idx]
        cum += c
        print(f"    {s:>4} {c:>+10.4f} {100.0 * cum / total:>7.1f}%")
    ...
    curve, ci_lo, ci_hi = grid_lca_inversion(
        fit=fit, switchers_kept=kept, base=base,
        phi_grid=phi_grid_marginal, type_one=TYPE_ONE,
    )
    ...
    if wald_gmm < threshold:
        margin = threshold - wald_gmm
        print(f"  ACCEPTED: Wald ({wald_gmm:.3f}) < threshold ({threshold:.3f}) "
              f"by margin {margin:.3f}")
    elif wald_gmm < threshold + 5.0:
        print(f"  BOUNDARY: Wald ({wald_gmm:.3f}) just above threshold "
              f"({threshold:.3f}); likely finite-sample noise at the joint level.")
        print(f"  Compare GMM Hansen J = {j_stat:.3f} on {j_df} dof; the two tests")
        print(f"  use different moment vectors and can disagree at the boundary.")
    else:
        print(f"  REJECTED: Wald ({wald_gmm:.3f}) >> threshold ({threshold:.3f}); "
              f"investigate spec.")
